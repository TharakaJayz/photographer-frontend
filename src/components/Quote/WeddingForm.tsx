...
interface Props {
    onSubmitCallback:(data:ContactFormWeddingsType) => void
}


export type ContactFormWeddingsType = {
    brideName: string;
    groomName: string;
    weddingDate: Date;
    Time: string;
...
const WeddingForm = (props: Props) => {

    const {
        register,
        handleSubmit,
        formState: { errors},
      } = useForm<ContactFormWeddingsType>();
    
      const submitHandler = handleSubmit((data:ContactFormWeddingsType) => {
      ...
      >
        ...
        <section className="w-full flex justify-between items-center gap-p5 lg2_:flex-col lg2_:items-start lg2_:gap-5">
          ...
          <div className="flex flex-col justify-start gap-1 w-p50 lg2_:w-full">
            ...
            {errors.Time && (
              <span className="text-xs text-white">{errors.Time.message}</span>
            )}
            <input
              type="text"
              placeholder="time"
        ...
        <section className="w-full flex justify-between items-center gap-p5 lg2_:flex-col lg2_:items-start lg2_:gap-5">
          <div className="flex flex-col justify-start gap-1 w-p50 lg2_:w-full">
            ...
            <input
              type="number"
              className="rounded-md px-2 py-1 outline-none "
              {...register("numOfGuests", { required: "This field is required" })}
            />
          </div>
          <div className="flex flex-col justify-start gap-1 w-p50 lg2_:w-full">
            <label htmlFor="">Makeup Artist</label>
            {errors.makeupArtist && (
              <span className="text-xs text-white">{errors.makeupArtist.message}</span>
            )}
            <input
              type="text"
              placeholder="makeup artist"
              className="rounded-md px-2 py-1 outline-none "
              {...register("makeupArtist", { required: "This field is required" })}
            />
          </div>
        </section>
        <section className="w-full flex justify-between items-center gap-p5 lg2_:flex-col lg2_:items-start lg2_:gap-5">
